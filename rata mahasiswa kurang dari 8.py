...
def ujian_dibawah_8(satu_mahasiswa):
    k = 0
    for nilai in satu_mahasiswa:
        k+=1
        if 0<nilai<8:
            break
    return k
nilai_tiap_mahasiswa = A[15]
simpan_k= []
for nilai_tiap_mahasiswa in A:
    ujian_k = ujian_dibawah_8(nilai_tiap_mahasiswa)
    simpan_k.append(ujian_k)
a = sum(simpan_k)/len(simpan_k)
print(a)